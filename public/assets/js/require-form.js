    ...
    var Form = {
        ...
        events: {
            ...
            faselect: function (form) {
                ...
                if ($(".fachoose_rid", form).size() > 0) {
                    $(".fachoose_rid", form).on('click', function () {
                        var that = this;
                        var multiple = $(this).data("multiple") ? $(this).data("multiple") : false;
                        var mimetype = $(this).data("mimetype") ? $(this).data("mimetype") : '';
                        parent.Fast.api.open("general/attachment/select?element_id=" + $(this).attr("id") + "&multiple=" + multiple + "&mimetype=" + mimetype, __('Choose'), {
                            callback: function (data) {
                                ...
                                if (input_id && data.multiple) {
                                    ...
                                    var ridArr = [];
                                    var inputObj = $("#" + input_id);
                                    var ridObj = $("#" + input_id + '-rid');
                                    var value = $.trim(inputObj.val());
                                    if (value !== "") {
                                        urlArr.push(inputObj.val());
                                        ridArr.push(ridObj.val());
                                    }
                                    urlArr.push(data.url);
                                    ridArr.push(data.rid);
                                    var result = urlArr.join(",");
                                    var rid_result = ridArr.join(",");
                                    if (maxcount > 0) {
                                        var nums = value === '' ? 0 : value.split(/\,/).length;
                                        var files = data.url !== "" ? data.url.split(/\,/) : [];
                                        var remains = maxcount - nums;
                                    ...
                                    inputObj.val(result).trigger("change");
                                    ridObj.val(rid_result).trigger("change");
                                } else {
                                    $("#" + input_id).val(data.url).trigger("change");
